...
# wait for mysql
while ! mariadb -h$DB_HOST -u$DB_USER -p$DB_PASSWORD $DB_DATABASE &>/dev/null; do
    sleep 3
done
...
if [ ! -f "/var/www/html/index.html" ]; then
    ...
    wp config create --dbname=$DB_DATABASE --dbuser=$DB_USER --dbpass=$DB_PASSWORD --dbhost=$DB_HOST --dbcharset="utf8" --dbcollate="utf8_general_ci" --allow-root
