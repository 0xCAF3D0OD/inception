...
# wait for mysql
while ! mariadb -h$DB_HOST -u$DB_USER -p$DB_PWD $DB_NAME &>/dev/null; do
    sleep 3
done
...
if [ ! -f "/var/www/html/index.html" ]; then
    ...
    wp core install --url=/Users/$USER/data/wordpress --title=$WP_TITLE --admin_user=$WP_ADMIN_USER --admin_password=$WP_ADMIN_PWD --admin_email=$WP_ADMIN_EMAIL --skip-email --allow-root
						#$DOMAIN_NAME/
